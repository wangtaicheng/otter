...
public abstract class AbstractProcessListener extends ArbitrateLifeCycle implements ProcessListener {
    ...
    protected static final ZkClientx ZOOKEEPER = ZooKeeperClient.getInstance();
    /**
     * 有响应的processId列表
     */
     ...
     * 最小并行度
     */
    private static final int MIN_PARALLELISM = 100;
    ...
    public AbstractProcessListener(Long pipelineId) {
        ...
        if (size < MIN_PARALLELISM) {
            size = MIN_PARALLELISM;
        }
    ...
    public Long waitForProcess() throws InterruptedException {
        ...
        Long processId = replyProcessIds.take();
            ...
            if (!processIds.contains(replyId)) {
                ...
                if (!CollectionUtils.isEmpty(processIds)) {
                    ...
                    if (processId > (Long) replyId) {
                        ...
                        logger.info("## {} remove reply id [{}]", ClassUtils
                                .getShortClassName(this.getClass()), replyId);
