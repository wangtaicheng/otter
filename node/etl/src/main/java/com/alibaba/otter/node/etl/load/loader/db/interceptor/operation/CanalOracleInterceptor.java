...
public class CanalOracleInterceptor extends AbstractOperationInterceptor {
    ...
    public static final String MERGE_OF_ORACLE_SQL = "merge /*+ use_nl(a b)*/ into {0} a using (select ? as id , ? as {1} from dual) b on (a.id=b.id)"
            + " when matched then update set a.{1}=b.{1}"
            + " when not matched then insert (a.id , a.{1}) values (b.id , b.{1})";

    public static final String MERGE_OF_ORACLE_INFO_SQL = "merge /*+ use_nl(a b)*/ into {0} a using (select ? as id , ? as {1} , ? as {2} from dual) b on (a.id=b.id)"
    ...
    public CanalOracleInterceptor() {
        super(MERGE_OF_ORACLE_SQL, MERGE_OF_ORACLE_INFO_SQL);
    }
